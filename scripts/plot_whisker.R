...
# Last Modified : Fri Oct 25 19:01:20 2024
#
#=========================================
...
nskips<-cbind(paste0("noN",1:20),
              paste0("N[skip]",1:20))
fid<-data.frame(sampler="nautilus",iamodel="massdep",blind="_A[_.]",type='mode',statistic='cosebis',nmax='NA',nskip="NA",boltzmann='cosmopower_hm2020')
...
for (blind in c("A","B","C")) { 
  ...
  png(file=paste0('whisker_plot_',blind,'.png'),height=4*220,width=8*220,res=220)
  print(length(levels(factor(file.dt[[groupby[1]]]))))
  layout(t(c(1,1:length(levels(factor(file.dt[[groupby[1]]]))))))
  par(mar=c(4,1,2,1))
