...
for nn in range(1,Nmax+1):
    ...
    tmn[:,1] /= 2 
    if nn < 10:
        file_tpn = "/Users/benjamin/OneCovariance/covariance_arb_summary/Tplus_%.2f-%.2f_0%d.table"%(tmin,tmax,nn)
        file_tmn = "/Users/benjamin/OneCovariance/covariance_arb_summary/Tminus_%.2f-%.2f_0%d.table"%(tmin,tmax,nn)
        file_Wn = "/Users/benjamin/OneCovariance/covariance_arb_summary/WnLog_%.2f-%.2f_0%d.table"%(tmin,tmax,nn)
    else:
        file_tpn = "/Users/benjamin/OneCovariance/covariance_arb_summary/Tplus_%.2f-%.2f_%d.table"%(tmin,tmax,nn)
        file_tmn = "/Users/benjamin/OneCovariance/covariance_arb_summary/Tminus_%.2f-%.2f_%d.table"%(tmin,tmax,nn)
        file_Wn = "/Users/benjamin/OneCovariance/covariance_arb_summary/WnLog_%.2f-%.2f_%d.table"%(tmin,tmax,nn)
    np.savetxt(file_tpn,tpn)
    np.savetxt(file_tmn,tmn)    
    if get_W_ell_as_well:
        np.savetxt(file_Wn, np.array([np.log(ell),result_Well]).T, header=' %f %f %d 1'%(np.log(ell_min), np.log(ell_max), N_ell))
