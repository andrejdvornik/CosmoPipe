...
# Last Modified : Fri 22 Sep 2023 10:22:23 AM CEST
#
#=========================================
...
input.catalogue<-helpRfuncs::read.file(input.file,verbose=T)
#}}}

if (length(input.catalogue[[weight.name]])!=0){ 
  warning("There is already a column called",weight.name,"in the input catalogue! It will be renamed!")
  input.catalogue[[paste0(weight.name,'_old')]]<-input.catalogue[[weight.name]]
}

#Initialise weight column
input.catalogue[[weight.name]]<-0
