...
neff_file=open("@RUNROOT@/@STORAGEPATH@/covariance/input/@SURVEY@_neff_sigmae.txt")
...
for blind in '@BLIND@':
	name_in='@SURVEY@_@ALLPATCH@_'+cat_version_in+'_blind'+blind
	name_out = '@SURVEY@_@ALLPATCH@_'+cat_version_out+'_blind'+blind
	OutputFileName='@RUNROOT@/@STORAGEPATH@/covariance/inputs/'+name_out+'.fits'
	OneBin_nofzFileName='@RUNROOT@/@STORAGEPATH@/covariance/inputs/'+name_out+'_single_bin.fits'
	input_files=[]
	for bin1 in range(nBins_KIDS):
		fileNameInput='@RUNROOT@/@STORAGEPATH@/covariance/inputs/'+name_in+str(bin1+1)+'@NZFILESUFFIX@'
