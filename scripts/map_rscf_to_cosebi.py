...
parser.add_argument('--thetamin', dest='thetamin', type=str, required=True, help="Minimum theta in arcmin")
parser.add_argument('--thetamax', dest='thetamax', type=str, required=True, help="Maximum theta in arcmin")
...
tmin_mm = float(args.thetamin) #theta_min in arcmin
tmax_mm = float(args.thetamax) #theta_max in armin
...
output_cov_EB = args.output_cov_EB
...
theta_min_mm = float(args.thetamin)
theta_max_mm = float(args.thetamax)
...
theta = np.loadtxt(tfoldername+'/'+tplusfile+'_'+thetaRange+'_'+str(1).zfill(zf)+'.table',comments='#').T[0]
N_theta = len(theta)
...
for n in range(1,Nmax_mm+1):
    ...
    TplusFileName= tfoldername+'/'+tplusfile+'_'+thetaRange+'_'+str(n).zfill(zf)+'.table'
    TminusFileName= tfoldername+'/'+tminusfile+"_"+thetaRange+"_"+str(n).zfill(zf)+'.table'
...
np.savetxt(output_cov_EB+'/'+filename_cov,cov_new)
