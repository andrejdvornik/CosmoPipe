...
outfold=@RUNROOT@/@STORAGEPATH@/@DATABLOCK@/cosebis_dimless/
if [ ! -d ${outfold} ]
then 
  mkdir ${outfold}
fi 
...
normfile_dimless=${SRCLOC}/TLogsRootsAndNorms_dimless/Normalization_@BV:THETAMINXI@-@BV:THETAMAXXI@.table
rootfile_dimless=${SRCLOC}/TLogsRootsAndNorms_dimless/Root_@BV:THETAMINXI@-@BV:THETAMAXXI@.table
...
cosebiblock=`_read_datablock cosebis_dimless`
_write_datablock cosebis_dimless "`_blockentry_to_filelist ${cosebiblock}` ${Enfile} ${Bnfile}"
