...
gsl_matrix *moore_penrose_invers(gsl_matrix *m)
{
  int flag;
  const int dim1=(int)m->size1;
  const int dim2=(int)m->size2;

  gsl_matrix *mtrans=gsl_matrix_alloc(dim2,dim1);
  gsl_matrix *res=gsl_matrix_alloc(dim2,dim1);

  matrix_transpose(m,mtrans,dim1,dim2);

  if (dim1<=dim2) {
    gsl_matrix *sym=gsl_matrix_alloc(dim1,dim1);
    gsl_matrix *sym_inv=gsl_matrix_alloc(dim1,dim1);
    mattimesmat_gen(m,mtrans,sym,dim1,dim2,dim1);
    flag=covinvers(sym,sym_inv,dim1);
    mattimesmat_gen(mtrans,sym_inv,res,dim2,dim1,dim1);
    gsl_matrix_free(sym);
    gsl_matrix_free(sym_inv);
  }
  else {
    gsl_matrix *sym=gsl_matrix_alloc(dim2,dim2);
    gsl_matrix *sym_inv=gsl_matrix_alloc(dim2,dim2);
    mattimesmat_gen(mtrans,m,sym,dim2,dim1,dim2);
    flag=covinvers(sym,sym_inv,dim2);
    mattimesmat_gen(sym_inv,mtrans,res,dim2,dim2,dim1);
    gsl_matrix_free(sym);
    gsl_matrix_free(sym_inv);
  }

  if (flag) {
    printf("Error in routine 'moore_penrose_invers': matrix does not have full rank.\n");
    exit(-1);
  }
