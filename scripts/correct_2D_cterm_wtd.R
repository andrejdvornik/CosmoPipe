...
# Last Modified : Mon 29 Apr 2024 10:52:52 PM CEST
#
#=========================================
...
while (length(inputs)!=0) {
  ...
  if (inputs[1]=='-i') { 
    ...
    #Read the weight column name /*fold*/ {{{
    inputs<-inputs[-1]
    wt.name<-inputs[1]
    inputs<-inputs[-1]
    #/*fold*/}}}
...
#Check that the wt.name varaible is in the catalogue 
if (!wt.name %in% colnames(cat)) {  
  stop(paste(wt.name,"variable is not in provided catalogue!")) 
}

cat<-helpRfuncs::read.file(input.cat,cols=c(x.name,y.name,e1.name,e2.name,wt.name))

#Set up the cut object (for faster splitting) {{{
tmp<-data.frame(x=cat[[x.name]],y=cat[[y.name]],e1=cat[[e1.name]],e2=cat[[e2.name]],wt=cat[[wt.name]])
#}}}
...
denom<-with(tmp,tapply(wt,list(x=cut(x, breaks=x.bin, include.lowest=T),
                               y=cut(y, breaks=y.bin, include.lowest=T)),sum))
cat('computing 2D c1\n')
c1numer<-with(tmp,tapply(e1*wt,list(x=cut(x, breaks=x.bin, include.lowest=T),
                                    y=cut(y, breaks=y.bin, include.lowest=T)),sum))
c1mean<-c1numer/denom
cat('computing 2D c1\n')
c2numer<-with(tmp,tapply(e2*wt,list(x=cut(x, breaks=x.bin, include.lowest=T),
                                    y=cut(y, breaks=y.bin, include.lowest=T)),sum))
c2mean<-c2numer/denom
#}}}
