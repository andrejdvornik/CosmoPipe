...
from mcmc_tools import load_chain, cosmosis_names
...
parser.add_argument("--massdep_cov", dest="massdep_cov",
    help="Covariance matrix between halo masses for mass-dependent IA model", required=True)
...
try:
    ...
    massdep_cov = np.loadtxt(massdep_cov)[2:,:][:,2:]
    pa_uncorr = ['uncorr_M1', 'uncorr_M2', 'uncorr_M3', 'uncorr_M4', 'uncorr_M5', 'uncorr_M6']
    ...
    pars = np.zeros((n,6))
    for i in range(n):
        pars[i] = L @ massdep_chain[0][co_uncorr].to_numpy()[i]

    for i in range(6):
        massdep_chain[0][co_derived[i]] = pars.T[i]
...
plt.tight_layout()
plt.savefig(output_path + "/plot_ia_amplitude_posteriors.pdf")
plt.close()
