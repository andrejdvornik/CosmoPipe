...
obj_cat = obj_cat[(obj_cat[col_ZB]>Z_B_edges[0]) & (obj_cat[col_ZB]<=Z_B_edges[-1])]
...
# Columns of interest
columns = ['SeqNr', col_weight, col_snr, col_ZB, col_e1, col_e2, col_psf_e1, col_psf_e2, 'R']
...
for ibin_R in range(N_R):
    ...
    # bin in R
    obj_cat.loc[mask_binR,'bin_snr'] = pd.qcut(obj_cat[col_snr][mask_binR], N_SNR, 
...
obj_cat.loc[cata_corr['AlphaRecal_index'].to_numpy(),'AlphaRecalD1_alpha1_est'] = cata_corr['AlphaRecalD1_alpha1_est'].to_numpy() 
obj_cat['AlphaRecalD1_alpha1_err_est'] = np.zeros(len(obj_cat)).astype(np.float64) 
obj_cat.loc[cata_corr['AlphaRecal_index'],'AlphaRecalD1_alpha1_err_est'] = cata_corr['AlphaRecalD1_alpha1_err_est'].to_numpy() 
...
obj_cat.loc[cata_corr['AlphaRecal_index'],'AlphaRecalD1_alpha2_est'] = cata_corr['AlphaRecalD1_alpha2_est'].to_numpy() 
obj_cat['AlphaRecalD1_alpha2_err_est'] = np.zeros(len(obj_cat)).astype(np.float64) 
obj_cat.loc[cata_corr['AlphaRecal_index'],'AlphaRecalD1_alpha2_err_est'] = cata_corr['AlphaRecalD1_alpha2_err_est'].to_numpy() 
...
for ibin_ZB in range(len(Z_B_edges)-1):
    ...
    # bin in R
    obj_cat.loc[mask_binZB, 'bin_R'] = pd.qcut(obj_cat['R'][mask_binZB], N_R, 
                                    labels=False, retbins=False)
    ...
    for ibin_R in range(N_R):
        ...
        # bin in R
        obj_cat.loc[mask_binZB & mask_binR, 'bin_snr'] = pd.qcut(obj_cat[col_snr][mask_binZB & mask_binR], N_SNR, 
                                                    labels=False, retbins=False)
        del mask_binR
    del mask_binZB
...
obj_cat.loc[cata_corr['AlphaRecal_index'],'AlphaRecalD2_alpha1'] = cata_corr['AlphaRecalD2_alpha1'].to_numpy() 
obj_cat['AlphaRecalD2_alpha2'] = np.zeros(len(obj_cat)).astype(np.float64) 
obj_cat.loc[cata_corr['AlphaRecal_index'],'AlphaRecalD2_alpha2'] = cata_corr['AlphaRecalD2_alpha2'].to_numpy() 
obj_cat['AlphaRecalD2_const1'] = np.zeros(len(obj_cat)).astype(np.float64) 
obj_cat.loc[cata_corr['AlphaRecal_index'],'AlphaRecalD2_const1'] = cata_corr['AlphaRecalD2_const1'].to_numpy() 
obj_cat['AlphaRecalD2_const2'] = np.zeros(len(obj_cat)).astype(np.float64) 
obj_cat.loc[cata_corr['AlphaRecal_index'],'AlphaRecalD2_const2'] = cata_corr['AlphaRecalD2_const2'].to_numpy()
obj_cat['AlphaRecalD2_e1'] = np.zeros(len(obj_cat)).astype(np.float64) 
obj_cat.loc[cata_corr['AlphaRecal_index'],'AlphaRecalD2_e1'] = cata_corr['AlphaRecalD2_e1'].to_numpy()
obj_cat['AlphaRecalD2_e2'] = np.zeros(len(obj_cat)).astype(np.float64) 
obj_cat.loc[cata_corr['AlphaRecal_index'],'AlphaRecalD2_e2'] = cata_corr['AlphaRecalD2_e2'].to_numpy()
...
output_columns = [c for c in obj_cat.columns if 'AlphaRecal' in c and 'weight' not in c]
