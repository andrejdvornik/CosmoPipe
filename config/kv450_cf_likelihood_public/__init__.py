...
#
# Designed for using data from Hildebrandt et al. 18XX.XXXX
#
##############################################################
...
class kv450_cf_likelihood_public(Likelihood):
    ...
    def __init__(self, path, data, command_line):
        ...
        self.xi_obs = np.zeros(self.ntheta * self.nzcorrs * 2)
        ...
        if os.path.exists(xipm_file_path):
            ...
            print 'Loaded xi_pm values from: \n', xipm_file_path
        else:
        ...
        self.xi_obs = self.__get_xi_obs(temp)

        # Read covariance matrix
        ndim = (self.ntheta) * (self.nzbins) * (self.nzbins + 1)
        #print ndim, self.ntheta, self.nzbins
        ...
        fname = self.data_directory + 'kids450_xipm_4bin_cut.dat'
        np.savetxt(fname, self.xi_obs[self.mask_indices])

        # apply equation 12 from Hildebrandt et al. 2017 to covmat:
        # this assumes that m-correction was already applied to covariance and data-vector!
        fname = self.data_directory + 'cov_matrix_ana_cut.dat'
        covmat = covmat[np.ix_(self.mask_indices, self.mask_indices)]
        np.savetxt(fname, covmat)
        print 'Saved covmat cut down to requested scales to: \n', fname
        # deprecated now!
        '''
        ...
        if self.use_cterm_function:
            fname = os.path.join(self.data_directory, self.cterm_function_file)
            ...
            print 'Loaded theta-dependent c-term function from: \n', fname
            #print self.xip_c_per_zbin.shape

        ## load patch/random correction
        #patchrandcorr_file_path = os.path.join(
        #    self.data_directory, self.patchrandcorr_file)
        #if os.path.exists(patchrandcorr_file_path):
        #    patchrandcorr = np.loadtxt(patchrandcorr_file_path)[:, 1:]
        #    print 'Loaded patch/rand correction values from: \n', patchrandcorr_file_path
        #    #print patchrandcorr
        #else:
        #    raise io_mp.LikelihoodError("File not found:\n %s"%patchrandcorr_file_path)
        #self.xipm_patchrandcorr = self.__get_xi_obs(patchrandcorr)
    ...
    def loglkl(self, cosmo, data):
        ...
        for index_l in xrange(self.nlmax):
            for index_z in xrange(1, self.nzmax):
                ...
                '''
                if self.method_non_linear_Pk == 'halofit':
                    ...
                    #else:
                    #    self.pk[index_l, index_z] = pk_dm * self.baryon_feedback_bias_sqr(k_in_inv_Mpc / self.small_h, self.z_p[index_z])
                    #    self.pk_lin[index_l, index_z] = pk_lin_dm * self.baryon_feedback_bias_sqr(k_in_inv_Mpc / self.small_h, self.z_p[index_z])
                else:
        ...
        #print self.xipm_patchrandcorr, self.xipm_patchrandcorr.shape, self.xipm_patchrandcorr.dtype, self.xipm_patchrandcorr[0]
        ...
        #self.xi = (self.xi * dm_plus_one_sqr_obs / self.xipm_patchrandcorr) + xipm_c + dc_sqr
        self.xi = self.xi * dm_plus_one_sqr_obs + xipm_c + dc_sqr

        if self.write_out_theory:
            xi_p_theo, xi_m_theo = self.__get_xi_p_and_xi_m(self.xi)
            fname = os.path.join(self.data_directory, self.xipm_theory_filename)
            header = 'theta [arcmin], '
            for i in xrange(self.nzbins):
                for j in xrange(i, self.nzbins):
                    header += 'xi_+/-(theta, z{:}xz{:}), '.format(i + 1, j + 1)
            # remove last space and ',':
            header = header[:-2]
            #print self.theta_bins.shape, xi_p_theo.shape, xi_m_theo.shape
            savedata = np.column_stack((self.theta_bins, np.concatenate((xi_p_theo, xi_m_theo))))
